...
API Key 面向调用 WebFetch 的下游业务系统，用于证明调用方身份；它不用于登录被抓取的网站。下游系统通过 `Authorization: Bearer <API_KEY>` 请求头传递凭据，并应将凭据保存在环境变量或密钥管理系统中，不得硬编码或写入日志。

首版交付状态：系统使用配置注入的共享 bootstrap API Key 完成认证。FR-AUTH-003～005 描述的是后续调用方管理目标；在这些需求实现前，多个下游系统共享同一个 Key，无法按系统单独吊销、限流或审计。

| 编号 | 优先级 | 需求 |
|---|---:|---|
| FR-AUTH-001 | P0 | 所有业务接口必须使用 API Key 认证，健康检查除外。 |
| FR-AUTH-002 | P0 | API Key 仅保存不可逆哈希，不在日志中记录明文。 |
| FR-AUTH-003 | P1 | 每个 API Key 必须绑定调用方名称、启用状态和请求配额。 |
...
| 编号 | 优先级 | 需求 |
|---|---:|---|
| FR-CACHE-001 | P0 | 缓存键至少包含规范化 URL、抓取模式、Profile、请求方法、影响内容的关键请求头摘要和 `save_artifact` 三态值（`null`、`false`、`true`）。不同 artifact 保存要求不得共享缓存键。 |
...
- 发布健康检查成功后必须自动清理旧版本；默认保留最近 5 个版本，保留数量可配置且不得小于 2，当前运行版本不得被删除。
